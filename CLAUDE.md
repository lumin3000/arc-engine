...
1. **bedrock/ 不得依赖任何 bindings/** — S2.1 完成回调注入后，bedrock/js_runtime.c 已不再 `#include` 任何 bindings/*.h。引擎 binding 通过 `Engine_Config.register_main_js_bindings` / `register_render_js_bindings` 两个函数指针注入，入口由消费者提供（通常直接传 `src/bindings/common_bindings.h` 的 `arc_register_main_js_bindings` / `arc_register_render_js_bindings`）。通用 binding 的注册时机依赖 jtask 对 `js_init_message_module` 的 weak-link — jtask 会在每个 worker ctx + bootstrap ctx 都调用它，registrar 就在其尾部执行，所有 worker 的 `globalThis.loader/graphics/...` 都从这条路径来。**游戏专属 binding（如 blood_bindings）永远不得在 bedrock/ 或 bindings/ common_bindings 里出现**，只能由消费者在自己的 `Engine_Config.on_register_js_bindings` 里注册。
...
3. **shader 业务分支禁止硬编码** — 核心 shader (`src/shader/shader_core.glsl`) 通过 `@include user_fragment.glsl` 提供扩展点；引擎仓里的 `user_fragment.glsl` 是通用 stub（只处理 tex_index 0..3 + FLAG_background_pixels + col_override）。游戏在自己仓的 `src/shader/user_fragment.glsl` 写专属片段（terrain / edge detect / 粒子等），构建时 Makefile 把游戏版 + engine 的 shader_utils.glsl + shader_core.glsl 拷到 `build/<platform>/shader_gen/` 再跑 sokol-shdc，sokol-shdc 的 `@include` 相对于 `_shader_core_c.glsl` 目录解析，找到游戏版 user_fragment。**引擎源码树不得被构建修改**。
