...
S3.6 Windows 端的 build/runtime 验证未在本迭代完成 —— Windows 机器（sjerr@172.16.1.199）的 SSH 在执行 S3.6 时不可达。

代码层面已经把已知的 POSIX-only 调用 guard 完毕（最后修了 `arc-mapgen/src/bindings/section_mesh.c` 的 `gettimeofday` 调用，改用引擎的 `seconds_since_init`）；arc-engine 自身在前轮 gunslinger Windows 测试中已通过；arc-mapgen 剩余风险只来自游戏专属 binding（cmd / data / entity / noise / ozz / section_mesh / terrain_buffer / thing_mesh）和它们的 game/ 依赖。当 Windows 机器恢复后跑一次 `make` 即可定位剩余问题；预计 0~少量 commit 修补。
