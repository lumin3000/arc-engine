...
// 双精度逐位对齐路径 (文末 *_d 函数) 禁止编译器把 a*b+c 合并成 FMA —— JS 引擎
// 不合并, 合并后末位不同, 世界生成 parity 判卷就红。clang 认这个 pragma;
// gcc 在 -std=c11 下默认已关 (-ffp-contract=off)。
...
// ============================================================================
// 双精度逐位对齐路径 — 对齐 00_perlin.ts / 05_ridged_multifractal.ts /
// 01_basic_modules.ts / 07_combiners.ts / 08_modifiers.ts (见 noise.h 说明)
// ============================================================================

// 与 00_perlin.ts NoiseUtils._randoms 逐字相同的 double 字面量 (脚本自动抽取)
static const double g_randoms_d[256 * 4] = {
...
// 对齐 00_perlin.ts makeInt32Range (ieeeRemainder 用 Math.round: floor(x+0.5))
static inline double make_int32_range_d(double value) {
    if (value >= 1073741824.0) {
        double rem = value - 1073741824.0 * floor(value / 1073741824.0 + 0.5);
        return 2.0 * rem - 1073741824.0;
    }
    if (value <= -1073741824.0) {
        double rem = value - 1073741824.0 * floor(value / 1073741824.0 + 0.5);
...
// 对齐 00_perlin.ts gradientNoise3D: n = ((X*ix + Y*iy + Z*iz + SEED*seed) & 0xFFFFFFFF) >>> 0
// JS 里四个乘积与和都在 double 内精确 (远小于 2^53), ToInt32 = 模 2^32 —— 用 int64 复现
static double gradient_noise_3d_d(double fx, double fy, double fz,
                                  double ix, double iy, double iz, int32_t seed) {
    int64_t sum = (int64_t)GENERATOR_NOISE_X * (int64_t)ix
                + (int64_t)GENERATOR_NOISE_Y * (int64_t)iy
                + (int64_t)GENERATOR_NOISE_Z * (int64_t)iz
                + (int64_t)GENERATOR_SEED * (int64_t)seed;
    uint32_t n = (uint32_t)(uint64_t)sum;
    n ^= (n >> 8);          // JS: n ^= (n >> 8); n &= 0xFF —— 低 8 位与移位符号无关
    n &= 0xFF;
    int idx = (int)n << 2;
    double gx = g_randoms_d[idx];
    double gy = g_randoms_d[idx + 1];
    double gz = g_randoms_d[idx + 2];
    double dx = fx - ix;
    double dy = fy - iy;
    double dz = fz - iz;
    return (gx * dx + gy * dy + gz * dz) * 2.12;
...
// 对齐 00_perlin.ts gradientCoherentNoise3D (Math.trunc 口径)
static double gradient_coherent_noise_3d_d(double x, double y, double z,
                                           int32_t seed, int quality) {
    double x0 = x > 0 ? trunc(x) : trunc(x) - 1;
    double x1 = x0 + 1;
    double y0 = y > 0 ? trunc(y) : trunc(y) - 1;
    double y1 = y0 + 1;
    double z0 = z > 0 ? trunc(z) : trunc(z) - 1;
    double z1 = z0 + 1;
    double xs, ys, zs;
    switch (quality) {
        case QUALITY_LOW:
            xs = x - x0; ys = y - y0; zs = z - z0;
            break;
        case QUALITY_HIGH:
            xs = map_quintic_s_curve(x - x0);
            ys = map_quintic_s_curve(y - y0);
    ...
    double n000 = gradient_noise_3d_d(x, y, z, x0, y0, z0, seed);
    double n100 = gradient_noise_3d_d(x, y, z, x1, y0, z0, seed);
    double n010 = gradient_noise_3d_d(x, y, z, x0, y1, z0, seed);
    double n110 = gradient_noise_3d_d(x, y, z, x1, y1, z0, seed);
    double n001 = gradient_noise_3d_d(x, y, z, x0, y0, z1, seed);
    double n101 = gradient_noise_3d_d(x, y, z, x1, y0, z1, seed);
    double n011 = gradient_noise_3d_d(x, y, z, x0, y1, z1, seed);
    double n111 = gradient_noise_3d_d(x, y, z, x1, y1, z1, seed);
    double nx00 = interpolate_linear(n000, n100, xs);
    double nx10 = interpolate_linear(n010, n110, xs);
    double nx01 = interpolate_linear(n001, n101, xs);
    double nx11 = interpolate_linear(n011, n111, xs);
...
// 对齐 00_perlin.ts Perlin.GetValue (static)
static double perlin_d(double x, double y, double z, const double *p) {
    double frequency = p[0], lacunarity = p[1], persistence = p[2];
    int octaveCount = (int)p[3];
    double seed = p[4];
    int quality = (int)p[5];
    int normalized = p[6] != 0.0, invert = p[7] != 0.0;
    double value = 0.0;
    double curPersistence = 1.0;
    x *= frequency; y *= frequency; z *= frequency;
    for (int i = 0; i < octaveCount; i++) {
        double nx = make_int32_range_d(x);
        double ny = make_int32_range_d(y);
        double nz = make_int32_range_d(z);
        int32_t curSeed = js_to_int32(seed + i);           // JS: (seed + i) & 0xFFFFFFFF
        double signal = gradient_coherent_noise_3d_d(nx, ny, nz, curSeed, quality);
...
// 对齐 05_ridged_multifractal.ts getValue
static double ridged_d(double x, double y, double z, const double *p) {
    double frequency = p[0], lacunarity = p[1];
    int octaveCount = (int)p[2];
    double seed = p[3];
    int quality = (int)p[4];
    const double *weights = p + 8;
    x *= frequency; y *= frequency; z *= frequency;
    double signal = 0.0, value = 0.0, weight = 1.0;
    const double offset = 1.0, gain = 2.0;
    for (int i = 0; i < octaveCount; i++) {
        double nx = make_int32_range_d(x);
        double ny = make_int32_range_d(y);
        double nz = make_int32_range_d(z);
        int32_t curSeed = js_to_int32(seed + i) & 0x7FFFFFFF;   // JS: (seed + i) & 0x7FFFFFFF
        signal = gradient_coherent_noise_3d_d(nx, ny, nz, curSeed, quality);
        signal = fabs(signal);
...
static double eval_node_d(const NoiseNodeD *nodes, int node_count, int idx,
                          double x, double y, double z) {
    if (idx < 0 || idx >= node_count) return 0.0;
    const NoiseNodeD *n = &nodes[idx];
    switch (n->type) {
    case NOISED_PERLIN: return perlin_d(x, y, z, n->params);
    case NOISED_RIDGED: return ridged_d(x, y, z, n->params);
    case NOISED_SCALE_BIAS:
        return eval_node_d(nodes, node_count, n->children[0], x, y, z) * n->params[0] + n->params[1];
    case NOISED_MULTIPLY: {
        double a = eval_node_d(nodes, node_count, n->children[0], x, y, z);
        double b = eval_node_d(nodes, node_count, n->children[1], x, y, z);
        return a * b;
    }
    case NOISED_CONST: return n->params[0];
    case NOISED_POWER: {
        double base = eval_node_d(nodes, node_count, n->children[0], x, y, z);
        double ex = eval_node_d(nodes, node_count, n->children[1], x, y, z);
        return pow(base, ex);
    }
    case NOISED_BLEND: {
        double value = eval_node_d(nodes, node_count, n->children[0], x, y, z);
        double value2 = eval_node_d(nodes, node_count, n->children[1], x, y, z);
        double position = (eval_node_d(nodes, node_count, n->children[2], x, y, z) + 1.0) / 2.0;
        return (1.0 - position) * value + position * value2;
    }
    case NOISED_ABS: return fabs(eval_node_d(nodes, node_count, n->children[0], x, y, z));
    case NOISED_ONE_MINUS: return 1.0 - eval_node_d(nodes, node_count, n->children[0], x, y, z);
    case NOISED_FILTER: {
        double v = eval_node_d(nodes, node_count, n->children[0], x, y, z);
        return (v >= n->params[0] && v <= n->params[1]) ? 1.0 : 0.0;
    }
    default: return 0.0;
    }
}
...
void noise_eval_points_d(const NoiseNodeD *nodes, int node_count, int root_idx,
    ...
    for (int i = 0; i < n; i++) {
        out[i] = eval_node_d(nodes, node_count, root_idx,
