...
typedef enum {
    ...
    INPUT_FLAG_REPEAT   = (1 << 3),
} Input_Flag;

#define TEXT_INPUT_BUFFER_SIZE 64

typedef struct {
    uint8_t keys[MAX_KEYCODES];
    float mouse_x, mouse_y;
    float scroll_x, scroll_y;

    char text_input[TEXT_INPUT_BUFFER_SIZE];
    int text_input_len;
} Input;

extern Input* input_state;

extern Input _actual_input_state;

extern void (*window_resize_callback)(int width, int height);
...
// 注入指针同步喂 ImGui 输入 (与 OS 事件同一通道, 保证 UI 悬停/穿透防护与真实输入一致)
