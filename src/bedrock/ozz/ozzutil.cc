...
#define OZZ_MAX_ANIMS 8

struct ozz_private_t {
    int index;
    ozz::animation::Skeleton skel;
    ozz::animation::Animation anims[OZZ_MAX_ANIMS];
    int num_anims = 0;
    int active_anim = 0;
...
int ozz_load_additional_animation(ozz_instance_t* ozz, const void* data, size_t num_bytes) {
    assert(state.valid && ozz && data && (num_bytes > 0));
    ozz_private_t* self = (ozz_private_t*) ozz;
    if (self->num_anims >= OZZ_MAX_ANIMS) return -1;
    ozz::io::MemoryStream stream;
    stream.Write(data, num_bytes);
    stream.Seek(0, ozz::io::Stream::kSet);
    ozz::io::IArchive archive(&stream);
    if (archive.TestTag<ozz::animation::Animation>()) {
        int idx = self->num_anims;
        archive >> self->anims[idx];
        self->num_anims++;
        return idx;
    }
    return -1;
...
void ozz_set_active_animation(ozz_instance_t* ozz, int anim_index) {
    assert(state.valid && ozz);
    ozz_private_t* self = (ozz_private_t*) ozz;
    if (anim_index >= 0 && anim_index < self->num_anims) {
        self->active_anim = anim_index;
    }
}
...
void ozz_load_mesh(ozz_instance_t* ozz, const void* data, size_t num_bytes) {
    ...
    if (archive.TestTag<ozz::sample::Mesh>()) {
        ...
        printf("[OZZ] mesh loaded: vertices=%zu, triangles=%d, joints=%d\n",
               num_vertices, self->num_triangle_indices / 3, self->num_skin_joints);
        ...
        for (int i = 0; i < (int)num_vertices; i++) {
            ...
            // UV coordinates
            if (!mesh.parts[0].uvs.empty()) {
                v->uv[0] = mesh.parts[0].uvs[i * 2 + 0];
                v->uv[1] = mesh.parts[0].uvs[i * 2 + 1];
            } else {
                v->uv[0] = 0.0f;
                v->uv[1] = 0.0f;
            }
...
void ozz_update_instance(ozz_instance_t* ozz, double seconds) {
    ...
    ozz_private_t* self = (ozz_private_t*) ozz;
    const ozz::animation::Animation& anim = self->anims[self->active_anim];
    const float anim_duration = anim.duration();
